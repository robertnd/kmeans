...
class KMeans {
  ...
  /*def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    if (points.isEmpty && means.isEmpty)  GenMap[Point, GenSeq[Point]]()
    else if (points.isEmpty) means.map(mean => mean -> GenSeq()).toMap
    else points.map(x => (findClosest(x, means), x)).groupBy(_._1).mapValues(f => f.map(_._2))     
  }*/
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.map(point => findAverage(point, classified.getOrElse(point, GenSeq())))
  }

  /*def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = 
    classified.map(f => findAverage(f._1, f._2) ).toList 
  */

  /*def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    newMeans.zip(oldMeans).map(zmeans => zmeans._1.squareDistance(zmeans._2) <= eta).fold(true)(_ && _)
  } */
